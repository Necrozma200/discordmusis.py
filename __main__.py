...
class Bot:
	def __init__(self, **kwargs):
      ...
      self.bot.lavalinkregion = kwargs["lavalinkregion"]
...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):
            ...
            bot.lavalink.add_node("0.0.0.0", self.bot.lavalinkport, self.bot.lavalinkpass, str(self.bot.lavalinkregion), 'default-node')
    ...
    async def play(self, ctx, *, query: str):
        ...
        if not player.is_playing:
            await player.play()
    @command.command(name="ping",aliases=['pings'])
    async def pin1(self,ctx):
      a=await ctx.send("Pinging `0000`")
